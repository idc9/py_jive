...
from lazymatpy import interface
...
class TestLinearOperator(object):
    ...
    def test_matvec(self):
        ...
        for matvecs in get_matvecs(self.A):
            ...
            assert_equal((2*A).T.matvec([1,1]), [10, 14, 18])
            assert_equal((2*A)*[[1],[1],[1]], [[12],[30]])
        ...
        for matvecsA, matvecsB in product(get_matvecs(self.A),
            ...
            assert_equal((A*B).T.matvec([1,1]), [71,92])
        ...
        for matvecsC in get_matvecs(self.C):
            ...
            assert_equal((C**2).T.matvec([1,1]), [22,32])
    ...
    def test_transpose_basic(self):
        ...
        A = interface.LinearOperator(shape=M.shape,
                                     matvec=lambda x: np.dot(M, x).reshape(M.shape[0]),
                                     rmatvec=lambda x: np.dot(M.T, x).reshape(M.shape[1]),
                                     matmat=lambda x: np.dot(M, x),
                                     rmatmat=lambda x: np.dot(M.T, x))


        assert_equal(A.T * [1, 1], [5, 7, 9]) # multiply by vector
        assert_equal(A.T * np.array([[1, 2], [3, 4]]),  # multiply by matrix
                     np.array([[13, 18], [17, 24],[21, 30]]))


    def test_r_defaulting(self):
        M = np.array([[1,2,3],
                      [4,5,6]])

        # check rmatmat defaults to rmatvec
        A = interface.LinearOperator(shape=M.shape,
                                     matvec=lambda x: np.dot(M, x).reshape(M.shape[0]),
                                     rmatvec=lambda x: np.dot(M.T, x).reshape(M.shape[1]))
        ...
        # check rmatvec defaults to rmatmat
        A = interface.LinearOperator(shape=M.shape,
                                     matvec=lambda x: np.dot(M, x).reshape(M.shape[0]),
                                     rmatvec=lambda x: np.dot(M.T, x).reshape(M.shape[1]))
...
def test_identity():
    ...
    assert_equal(ident.T.dot([1, 2, 3]), [1, 2, 3])

    assert_raises(ValueError, interface.IdentityOperator, (3, 4))

    # check initializing identity with int
    assert_equal(interface.IdentityOperator(3) * [1, 2, 3], [1, 2, 3])
